...
def save_trajectory_data_to_csv(scene_files, base_dir, save_dir):
    ...
    for file_idx, file_name in enumerate(scene_files):
        # 动态生成每个子场景的CSV文件路径
        scene_id = file_name.split('_')[5]  # 假设第4部分是子场景ID
        print(file_name.split('_'))
        csv_path = os.path.join(save_dir, f"trajectory_data_{scene_id}_prediction_k_valid4frames_social.csv")
