...
def train(model, train_gen, criterion, optimizer, device):
    ...
    with torch.set_grad_enabled(True):
        for batch_idx, data in enumerate(loader):
            ...
            velocity = data['velocity'].to(device)  # shape: (batch_size, time_steps, 2),v_x,v_y
            # print("velocity shape:", velocity.shape)
            # print("velocity:", velocity)

            check_none(input_traj=input_traj, target_traj=target_traj, first_history_index=first_history_index)
            
            all_goal_traj, cvae_dec_traj, KLD_loss, _ = model(
                input_traj, 
                # velocity = velocity,
                map_mask = None, 
                targets = target_traj, 
...
def test(model, test_gen, criterion, device):
    ...
    with torch.set_grad_enabled(False):
        for batch_idx, data in enumerate(loader):#for batch_idx, data in enumerate(val_gen):
            ...
            target_traj = data['target_y'].to(device)
            velocity = data['velocity'].to(device)  # shape: (batch_size, time_steps, 2),v_x,v_y
            scene = data['scene_name'].to(device)
