...
class ETHUCYDataLayer(data.Dataset):
    ...
    def __init__(self, args, split):
        ...
        if split == 'train':
            print("args.eth_root = ",args.eth_root)
        ...
        train_env.attention_radius[(node_type, node_type)] = 10.0 #10.0
        augment = False
        if split=='train':
        ...
        for index in range(len(self.dataset)):
            first_history_index, x_t, y_t, x_st_t, y_st_t,scene_name,timestep,velocity,node_id = self.dataset.__getitem__(index)
    ...
    def __getitem__(self, index):
        ...
        for idx in indices:
            ...
            ret['scene_name'].append(this_ret['scene_name'])
            ret['timestep'].append(this_ret['timestep'])
            ret['velocity'].append(this_ret['velocity'])  # 添加 velocity 数据
            ret['node_id'].append(this_ret['node_id'])

        ret['input_x'] = torch.stack(ret['input_x'])
        ret['input_x_st'] = torch.stack(ret['input_x_st'])
        ret['target_y'] = torch.stack(ret['target_y'])
        ret['first_history_index'] = torch.stack(ret['first_history_index'])
        ret['velocity'] = torch.stack(ret['velocity'])  # 堆叠 velocity 数据
        # to locate image
        ret['scene_name'] = ret['scene_name']
        ret['timestep'] = ret['timestep']
        ret['node_id'] = ret['node_id']
        # print("rest shape", ret.shape)
        return ret
    ...
    def getitem_one(self, index):
        ...
        all_t = torch.cat((x_t[:, :2], y_t), dim=0)
        y_t = self.get_target(all_t, 0, self.args.enc_steps, self.args.enc_steps, self.args.dec_steps)
        # print("after y_t.shape:",y_t.shape)
        # print("after y_t",y_t)
        
        # 提取 velocity
        vx = x_t[:, 2:4]  # 根据具体的列索引提取速度信息
        ret['first_history_index'] = first_history_index
        ret['input_x'] = x_t
        ret['input_x_st'] = x_st_t
        ret['target_y'] = y_t
        ret['target_y_st'] = y_st_t
        ret['velocity'] = vx  # 添加 velocity 到返回字典
        ret['scene_name'] = scene_name
        ret['timestep'] = timestep
        ret['node_id'] = node_id
        return ret
